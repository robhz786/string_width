...
codepoint_properties obtain_props(UChar32 ch) {

    if (ch <= 0x007E) {
        if (0x0020 <= ch) return {category::other, 1};
        if (0x000D == ch) return {category::cr, 1};
        if (0x000A == ch) return {category::lf, 1};
        return {category::control, 1};
    }
    int ch_width = 1;
...
int main() {
    ...
    for (std::uint32_t ch = 0; ch < 0x110000; ++ch) {
        auto obtained = obtain_props(ch);

        int obtained_width = obtained.width;
        ...
        if (obtained_width != expected_width || obtained_category != expected_category) {
            printf("For codepoint U+%.4X:", ch);
            ++failures_count;
        }
        if (obtained_category != expected_category) {
            printf( "    Got category \'%s\', expected \'%s\'.\n"
                  , to_str(obtained_category)
                  , to_str(expected_category) );
        }
        if (obtained_width != expected_width) {
            printf("     Got width = %d (expected %d).\n", obtained_width, expected_width);
        }
